...
const Apps = () => {
  ...
  return (
    <>
      ...
      {(data && data[0].total > 0)
        ? <div className='relative grid content-start grid-cols-1 gap-4 px-12 pt-2 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5 2k:grid-cols-6 grow'>
          {data.map(({ data: apps }) => apps.map(app => (
            <AppCard key={app.id} app={app} onRefresh={mutate} />
          )))}
        </div>
        : <div className='relative grid content-start grid-cols-1 gap-4 px-12 pt-2 overflow-hidden sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5 2k:grid-cols-6 grow'>
          <NoAppsFound />
        </div>}
      <CheckModal />
...
function NoAppsFound() {
  ...
  return (
    <>
      {renderDefaultCard()}
      <div className='absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-gradient-to-t from-background-body to-transparent'>
